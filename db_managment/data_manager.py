...
class DataManager:
    ...
    def _get_connection(self):
        try:
            ...
            logger.error(
                f'{datetime.now().strftime("%Y-%m-%d %H:%M:%S")}: Error connecting to database: {e}'
            )
    ...
    def get_all_rows(self, table_name):
        ...
        try:
            ...
            logger.error(
                f'{datetime.now().strftime("%Y-%m-%d %H:%M:%S")}: Error loading rows from table {table_name}: {e}'
            )
            conn.rollback()
    ...
    def clear_table(self, table_name):
        ...
        try:
            ...
            logger.error(
                f'{datetime.now().strftime("%Y-%m-%d %H:%M:%S")}: Error clearing table {table_name}: {e}'
            )
            conn.rollback()
    ...
    def drop_table(self, table_name):
        ...
        try:
            ...
            logger.error(
                f'{datetime.now().strftime("%Y-%m-%d %H:%M:%S")}: Error dropping table {table_name}: {e}'
            )
            conn.rollback()
    ...
    def create_table(self, table_name):
        ...
        try:
            ...
            logger.error(
                f'{datetime.now().strftime("%Y-%m-%d %H:%M:%S")}: Error creating table {table_name}: {e}'
            )
            conn.rollback()
    ...
    def insert_new_estates(self, df):
        ...
        try:
            ...
            logger.error(
                f'{datetime.now().strftime("%Y-%m-%d %H:%M:%S")}: Error inserting offer: {e}'
            )
            conn.rollback()
    ...
    def _update_estate(self, df):
        ...
        try:
            ...
            for i in tqdm(range(len(df))):
                ...
                data_to_upload.append([r["timestamp"]])
            ...
            logger.error(
                f'{datetime.now().strftime("%Y-%m-%d %H:%M:%S")}: Error updating offer: {e}'
            )
            conn.rollback()
    ...
    def insert_new_price(self, df):
        ...
        try:
            ...
            for i in range(len(df)):
                r = df.iloc[i]
                data_to_upload.append(
                    [str(r["estate_id"]), str(r["price"]), str(r["crawled_at"])]
                )
